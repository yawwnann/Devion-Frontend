...
<template>
  <UDashboardPanel id="todos" class="!overflow-y-auto">
    <!-- Cover Image -->
    <div class="relative group">
      ...
      <div
        v-else
        class="h-52 w-full bg-gradient-to-br from-blue-600 via-blue-700 to-blue-900"
      />
    ...
    <div class="px-8 py-6">
      <div class="flex items-center justify-between mb-4">
        <div class="flex-1">
          ...
          <h1
            v-else
            class="text-4xl font-bold cursor-text hover:bg-zinc-100 dark:hover:bg-zinc-800/50 rounded-lg px-2 -mx-2 py-1 transition"
            @click="editingTitle = true"
          >
            {{ pageSettings.title }}
          </h1>
          <p v-if="weekRange" class="text-muted mt-2">
            {{ weekRange }}
          </p>
        </div>
        <UButton
    ...
    <div class="px-8 pb-4">
      <div class="flex gap-3">
        <select
          v-model="selectedDay"
          class="px-4 py-2 bg-white dark:bg-zinc-900 border border-zinc-300 dark:border-zinc-700 rounded-lg outline-none focus:ring-2 focus:ring-primary-500"
        >
    ...
    <div v-if="loading" class="px-8 pb-8">
      <div class="grid grid-cols-1 md:grid-cols-7 gap-4">
        <USkeleton v-for="i in 7" :key="i" class="h-64" />
      </div>
    </div>
    ...
    <div v-else class="px-8 pb-8">
      <div class="grid grid-cols-1 md:grid-cols-7 gap-4">
        <div v-for="day in days" :key="day" class="space-y-3">
          <!-- Day Header -->
          <div
            class="font-semibold text-sm uppercase tracking-wider text-muted"
          >
            {{ day }}
          </div>
          ...
          >
            <template #item="{ element: todo }">
              <div
                class="group flex items-start gap-2 p-3 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg hover:border-primary-500 transition cursor-grab active:cursor-grabbing"
              >
                <input
                  type="checkbox"
                  :checked="todo.isCompleted"
                  class="mt-0.5 size-4 rounded border-zinc-300 text-primary-600 focus:ring-primary-500"
          ...
          <div
            ...
            class="p-4 text-center text-sm text-muted border-2 border-dashed border-zinc-200 dark:border-zinc-800 rounded-lg"
          >
            No to-dos
          </div>
        </div>
