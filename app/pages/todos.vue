...
const enableAddTask = async (columnId: string) => {
...
const confirmAddTask = async (columnId: string) => {
  ...
  newColumnTaskTitle.value = ""; // Clear for rapid entry
...
const getPriorityColor = (priority?: string) => {
  switch (priority) {
    ...
    default:
      return "bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400"; // MEDIUM
  }
};
...
<template>
      ...
      <div class="h-full flex flex-col overflow-hidden">
        ...
        <div class="shrink-0 relative group">
          <div
            ...
            class="h-48 w-full bg-cover bg-center transition-all duration-700"
            :style="{ backgroundImage: `url(${pageSettings.cover})` }"
          >
            <div
              class="absolute inset-0 bg-linear-to-t from-zinc-50 dark:from-zinc-950 to-transparent opacity-80"
            />
          </div>
          <div v-else class="h-32 w-full bg-green-500 opacity-90">
            <div
              class="absolute inset-0 bg-linear-to-t from-zinc-50 dark:from-zinc-950 to-transparent"
            />
          </div>

          <!-- Cover Controls -->
          <div
            class="absolute bottom-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity z-10"
          >
            <button
        ...
        <div class="flex-1 overflow-x-auto overflow-y-hidden p-4 sm:p-6">
          <div class="flex h-full gap-6 min-w-250">
            ...
            >
              ...
              <div class="flex items-center justify-between p-3 pb-2">
                <div class="flex items-center gap-2">
                  <div
                    ...
                    :class="[
                      col.dotClass,
                      `ring-${col.id === 'TODO' ? 'zinc' : col.id === 'SCHEDULED' ? 'blue' : col.id === 'IN_PROGRESS' ? 'orange' : 'green'}-500/20`,
                    ]"
                  />
              ...
              <div class="px-3 pb-2">
                <div
                  v-if="addingToColumn === col.id"
                  class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg p-3 shadow-sm"
                >
                  <textarea
                    ref="taskInputRef"
                    v-model="newColumnTaskTitle"
                    rows="2"
                    class="w-full bg-transparent border-none outline-none text-sm resize-none placeholder-zinc-400 font-medium"
                    placeholder="What needs to be done?"
                    @keydown.enter.exact.prevent="confirmAddTask(col.id)"
              ...
              <!-- Draggable Area -->
              <div
                class="flex-1 overflow-y-auto px-3 pb-3 scrollbar-hide relative"
              >
                  ...
                  <template #item="{ element: todo }">
                    <div
                      ...
                      :class="{
                        ...
                        'border-l-blue-500 dark:border-l-blue-500':
                          todo.priority === 'LOW',
                        'border-l-orange-400 dark:border-l-orange-400':
                    ...
                    >
                      ...
                      <div class="flex items-start justify-between mb-2">
                        ...
                        <div
                          class="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity absolute top-2 right-2 bg-white dark:bg-zinc-900 rounded-md shadow-sm border border-zinc-100 dark:border-zinc-800"
                        >
                          <button
                      ...
                      <!-- Title -->
                      <p
                        class="text-sm font-medium text-zinc-800 dark:text-zinc-100 mb-3 leading-snug"
                      >
                        {{ todo.title }}
                      </p>
                        ...
                        <div class="flex items-center gap-2">
                          ...
                          >
                            <UIcon
                              ...
                              class="size-3 text-zinc-600 dark:text-zinc-400 group-hover/gh:text-zinc-900 dark:group-hover/gh:text-white"
                            />
                            <span
                              v-if="commitCounts[todo.id]"
                              class="font-medium text-zinc-700 dark:text-zinc-300"
                            >
                              {{ commitCounts[todo.id] }}
                            </span>
                            <span v-else class="text-zinc-500"
                              >#{{ todo.githubIssueNumber }}</span
                            >
                          </button>
                        </div>
                      </div>
  ...
  <UModal v-model:open="showCropperModal">
    <template #content>
      <div
        class="bg-white dark:bg-zinc-950 w-full max-w-2xl mx-4 sm:mx-auto rounded-xl overflow-hidden"
      >
        <div
        ...
        >
          ...
          <UButton :loading="uploadingCover" @click="uploadCroppedCover">
            Save Cover
          </UButton>
        </div>
      </div>
